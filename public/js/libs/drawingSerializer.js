...
function deserializeDrawing (serData,SP)
{
	try
	{
		...
		if (recordings instanceof Array )
		{
			...
			{
				var rec = deserializeRecording(recordings[i],SP);
				if (rec != null)
					result.push(rec);
			}
		}
...
function deserializeRecording(recordingWrp,SP)
{
	var rec = new Recording(SP);
